...
class SegTree:
    ...
    def _init(self, node, start, end):
        ...
        mid=(start+end)//2
        self.tree[node]=(self._init(node*2,start,mid)*self._init(node*2+1, mid+1, end))%1000000007
        return self.tree[node]
    ...
    def _query(self, node, start, end, left, right):
        ...
        if left <= start and end <= right:
            return self.tree[node]
        mid = (start + end) // 2
        return (self._query(node * 2, start, mid, left, right) * self._query(node * 2 + 1, mid + 1, end, left, right)) % 1000000007
