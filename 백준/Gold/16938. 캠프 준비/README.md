### 문제 한줄평
더 문제 수가 많았으면 어려웠을 것 같은데, 문제 수가 적어서 단순히 그냥 combination으로 날먹 가능했던 문제. N의 최대치가 더 컸으면 더 재밌는 문제가 아닐까 싶었다.
문제가 최소 2개 최대 N개였기 때문에 range는 2~(N+1)개로 추릴 수 있음.
이를 이용해서 그냥 2~N개의 combination에서 max - min의 조건과 난이도의 합 조건에 대해서 추리면 되는 쉬운 문제이다.
